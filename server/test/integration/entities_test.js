...
describe("Entities", () => {
  ...
  function removeEntityFields(body) {
    ...
    for (const entity of body) {
      ...
      removeEntitySynonymFields(entity);
    }
  }

  function removeEntitySynonymFields(entity) {
  ...
  it("posting new entity works", function (done) {
    chai
      ...
      .end((err, res) => {
        ...
        removeEntitySynonymFields(body);
  ...
  it("intents get specific entity works", done => {
    chai
      .request(app)
      .get(`/api/entities/${ENTITY_2_SYNONYMS.name}`)
      .end((err, res) => {
        if (err) done(err)
        const body = res.body;
        res.should.have.status(200);
        ...
        removeEntityFields(body);
        assert.deepEqual(body, [ENTITY_2_SYNONYMS], "Returned entities doesn't match expected entities");
        done();
      });
  });
